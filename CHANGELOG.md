...
- 内置字体插件：支持选择网页强制字体组合。默认仍是 Monaco + 95% 微软雅黑；可改为 American Typewriter + 思源宋体，或自定义西文/中文字体。插件包管理页提供「字体」入口，设置对所有实例生效，已打开页面需刷新。
- 内置字体插件：启动时自动同步并加载本地「强制字体」插件，按嵌入文件做完整性校验，不允许商店覆盖或删除。官方 Chrome 137+ 改为调试口 `Extensions.loadUnpacked` 装入；不再使用 `--disable-extensions-except`，避免插件只写入配置却不启用。
- 内置字体插件：修复雅黑别名缺少 `unicode-range` 导致英文/数字被雅黑拉丁字形截胡、Monaco 不生效的问题；雅黑别名限定 CJK 区间，拉丁字符回退 Monaco（无 Monaco 时 Consolas 兜底），中文与中文标点保持 95% 缩放雅黑。
